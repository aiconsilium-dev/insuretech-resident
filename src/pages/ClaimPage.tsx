...
/* ─── AI 적산 데이터 (누수) ─── */
const LEAK_DAMAGE_COSTS: Record<
  string,
  {
...
function FacilityAIResult({
  ...
  claimId: string;
  onReady: (est: ApiEstimation) => void;
}) {
  const { mutate: runEst, data: estimation, isPending } = useRunEstimate();

  useEffect(() => {
    runEst(claimId, {
      onSuccess: (est) => onReady(est),
    });
...
export default function ClaimPage() {
  ...
  async function handleCreateAndSubmit(
    ...
    try {
      ...
      if (claimType === 'facility' || claimType === 'leak') {
        setStep(2);
      } else {
        setSubmitted(true);
      }
