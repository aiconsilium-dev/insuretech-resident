  ...
  return (
    <div className="animate-[fadeIn_0.25s_ease] pb-24">
      ...
      >
        ...
        <div className="text-sm text-white/75">{user.apt} {user.dong}동 {user.ho}호</div>
      </div>
          ...
          {MENU_ITEMS.map((item) => (
            <List.Item key={item.route} onClick={() => navigate(item.route)}>
              ...
              <div className="flex-1">
                <h4 className="text-[15px] font-semibold text-text-body mb-0.5">{item.label}</h4>
                <p className="text-[13px] text-text-muted">{item.desc}</p>
              </div>
              <svg className="text-text-dim" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
